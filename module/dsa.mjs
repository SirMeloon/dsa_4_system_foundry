...
import { dsaActor } from './documents/actor.mjs';
import { dsaItem } from './documents/item.mjs';
// Import sheet classes.
import { dsaActorSheet } from './sheets/actor-sheet.mjs';
import { dsaItemSheet } from './sheets/item-sheet.mjs';
...
Hooks.once('init', function () {
  ...
  CONFIG.Combat.initiative = {
    formula: '(@mu.value + @mu.value + @in.value + @ge.value) / 5 + 1d6',
    decimals: 2,
  };
  ...
  CONFIG.Actor.dataModels = {
    character: models.dsaCharacter,
    npc: models.dsaNPC
  }
  CONFIG.Item.documentClass = dsaItem;
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.dsa.rollItemMacro("${data.uuid}");`;
